...
const BlogModal = ({ blog, onClose }) => {
    // Lock background scroll — stop Lenis (smooth scroll engine) and also lock native scroll
    useEffect(() => {
    ...
    return createPortal(
        ...
        >
            <motion.div
                ...
                className="bg-white w-full max-w-4xl rounded-3xl shadow-2xl flex flex-col relative"
                style={{ maxHeight: 'min(90vh, 100%)' }}
                onClick={(e) => e.stopPropagation()}
            >
                {/* Header Section */}
                <div className="flex flex-col gap-4 p-8 border-b border-slate-100 bg-slate-50/50 pr-16 relative flex-shrink-0">
                    <button
                        onClick={onClose}
