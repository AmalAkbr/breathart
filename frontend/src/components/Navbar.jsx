...
const Navbar = () => {
  ...
  const getAvatarLetter = () => {
    if (user?.profileImage) return null; // will show image instead
      ...
      >
        ...
        >
          ...
          <div className="flex-1 flex items-center justify-end gap-2 md:gap-3">
            {isLoggedIn ? (
              isAdmin ? (
                <Link to="/admin" onMouseEnter={() => setHoveredItem(null)}>
                  <motion.button
                    initial={{ opacity: 0, x: 20 }}
                    animate={{ opacity: 1, x: 0 }}
                    whileHover={{ scale: 1.05 }}
                    whileTap={{ scale: 0.95 }}
                    className={`px-5 md:px-7 py-2.5 rounded-full font-bold text-sm transition-colors border hidden sm:flex items-center gap-2 ${isWhiteNav ? "border-amber-600/50 text-amber-600 hover:bg-amber-600/10" : "border-amber-400/50 text-amber-400 hover:bg-amber-400/10"}`}
                  >
                    <Lock size={16} /> Panel
                  </motion.button>
                </Link>
            ...
            <div className="flex items-center gap-2">
              {/* Desktop Menu Icon - Always visible on desktop */}
              <motion.button
                whileHover={{ scale: 1.1 }}
                whileTap={{ scale: 0.95 }}
                onClick={() => setDesktopMenuOpen(true)}
                className={`hidden lg:flex items-center justify-center cursor-pointer transition-colors duration-300 p-2 rounded-full ${isWhiteNav ? "text-blue-900 hover:bg-blue-100 hover:text-accent-blue" : "text-white hover:bg-white/10 hover:text-accent-cyan"}`}
                title="Open menu"
              >
                <Menu className="w-6 h-6" />
              </motion.button>

              {/* Mobile/Tablet hamburger - Always visible on mobile */}
              <motion.button
                whileHover={{ scale: 1.1 }}
                whileTap={{ scale: 0.95 }}
                  ...
                  <div className="flex flex-col gap-4">
                    ...
                    {isAdmin && (
                      <Link
                        ...
                        className={`text-xl font-bold py-2 transition-colors flex justify-center items-center group w-full text-center border-t-2 border-b-2 ${isWhiteNav ? "border-amber-600/30 text-amber-600 hover:text-amber-700" : "border-amber-400/30 text-amber-400 hover:text-amber-300"}`}
                      >
                        🔐 Admin Panel
                      </Link>
                    )}
